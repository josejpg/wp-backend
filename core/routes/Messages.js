...
router.get('/:_idEvent', (req, res) => {
    ...
    if (req.headers['authorization'] != null) {
        ...
        if (dataToken) {
            ...
            if (dataToken.exp < new Date().getTime()) {
                const params = {};
                if (req.params._idEvent != null) {
                    params.idEvent = req.params._idEvent;
                }
                Messages.findByEvent(params)
                    .then(listMessages => {
                        const listPromises = [];
                        listMessages.map(dataMessage => {
                            listPromises.push( Events.findById(dataMessage.evento)
                                .then(dataEvent => {
                                    dataMessage.evento = dataEvent;
                                    return dataMessage;
                                })
                                .then(dataMessage => {
                                    if (dataMessage.proveedor != null) {
                                        return Providers.findById(dataMessage.proveedor)
                                            .then(dataProvider => {
                                ...
                                .then(dataMessage => {
                                    if (dataMessage.cliente != null) {
                                        return Clients.findById(dataMessage.cliente)
                                            .then(dataClient => {
                                                dataMessage.cliente = dataClient[ 0 ];
                                                console.log( dataMessage.cliente.email );
